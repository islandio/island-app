...
module.exports.prototype.notify = function (subscriber, note, body, cb) {
  ...
  // console.log(subscriber, note, body);
  return;
  ...
  if (note.event.data.action.t === 'comment') {
    ...
    subject = note.event.data.action.a + ' '
        ...
        + note.event.data.target.n + ' at '
        + note.event.data.target.w;
    body = 'Let them know what you think.';
  }

  // Create a login key for this email.
  db.Keys.create({member_id: subscriber._id}, _.bind(function (err, key) {
    if (err) return cb(err);
 ...
 * @function cb
 */
module.exports.prototype.request = function (sub, cb) {
  cb = cb || function(){};
  if (!this.BASE_URI) return cb('BASE_URI required');
  ...
  Step(
    function () {
      ...
      // Get members.
      db.Members.read({_id: sub.subscriber_id}, this.parallel());
      db.Members.read({_id: sub.subscribee_id}, this.parallel());
    },
    function (err, subscriber, subscribee) {
      if (err) return cb(err);
      if (!subscriber) return cb('Could not find subscriber');
      if (!subscribee) return cb('Could not find subscribee');

      // Create a login key for this email.
      db.Keys.create({member_id: subscribee._id}, function (err, key) {
        if (err) return cb(err);

        // Setup the email.
        self.send({
          to: subscribee.displayName + ' <' + subscribee.primaryEmail + '>',
          from: self.FROM,
          subject: subscriber.displayName + ' wants to follow you'
        }, {
          file: 'request.jade',
          html: true,
          locals: {
            ...
            url: self.BASE_URI + '/request?t=' + key._id.toString()
          }
        }, cb);
      });
    }
