...
PubSub.prototype.publish = function (channel, topic, params, cb) {
  ...
  params = params || {};
  cb = cb || function(){};
  if (!this.sio || !channel || !topic || !params.doc) return cb('Invalid data');
  var self = this;

  // Publish raw doc (for static lists / tickers).
  self.sio.sockets.in(channel).emit(topic, com.client(params.doc));
        ...
        _.each(subs, function (sub) {
          ...
          db.Notifications.create({
            ...
            if ((email === true || email === 'true')
              ...
              self.mailer.notify(sub.subscriber, note, params.doc.body);
