...
var Client = exports.Client = function (webSock, backSock) {
  ...
  this.backSock.on('message', _.bind(function (data) {
    ...
    if ((topic === 'post.new' || topic === 'session.new' || topic === 'tick.new')
        && data.public !== false) {
...
Client.prototype.get8aUser = lib8a.searchUser;

/*
 * Get 8a ticks for an 8a userId, filling in documents for crag and ascent names
 */
Client.prototype.get8aTicks = function(userId, cb) {
  var db = this.db;
  lib8a.getTicks(userId, function(err, ticks) {
    ...
    Step (
      function() {
        ...
        _.each(ticks, function (tick) {
          db.Crags.findOne({name: tick.crag}, group());
        });
      },
      function (err, crags) {
        if (err) return this(err);
        _.each(crags, function (c, i) {
          if (c) ticks[i].crag = c;
        });
        var group = this.group();
        _.each(ticks, function (tick, i) {
          if (tick.crag._id) {
            db.Ascents.findOne({crag_id: tick.crag._id, name: tick.ascent}, group());
          }
          else {
            group()();
          }
      ...
      function (err, ascents) {
        if (err) cb(err);
        _.each(ascents, function (a, i) {

          // We know this ascent, easy-peasy.
          if (a) ticks[i].ascent = a;

          // Otherwise, create a new Ascent if we have a crag identified
          else if (ticks[i].crag._id) {
            var payload = {
              crag_id: ticks[i].crag._id,
              sector: ticks[i].ascentWall, 
              name: ticks[i].ascent,
              type: ticks[i].type,
              grades: ticks[i].grade
            }
          }
          // Delete this tick if no crag is identified
          else {
            ticks[i] = {}
          }
        });
