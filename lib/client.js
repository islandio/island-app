...
Client.prototype.get8aTicks = function(userId, cb) {
  ...
  lib8a.getTicks(userId, function(err, ticks) {
    ...
    var next = _.after(ticks.length, function(err) {
      cb(null, _.filter(ticks, function(t) { return t.hasOwnProperty('crag')}));
    });
    ...
    _.each(ticks, function (tick) {
      db.Crags.findOne({name: tick.crag}, function (err, crag) {
        ...
        tick.crag = crag;
        console.log('Searching for: ' + tick.ascent);
        db.Ascents.findOne({crag_id: tick.crag._id, name: tick.ascent},
            function (err, ascent) {
          console.log('Found?' + ascent);
          if (err) {
            delete tick.crag; 
            return next();
          }
          ...
          /*
            ...
            var payload = {
              crag_id: tick.crag._id,
              sector: tick.ascentWall, 
              name: tick.ascent,
              type: tick.type,
              grades: tick.grade
            };
            console.log('adding ascent:');
            console.log(payload);
            curl.request({
              url: app.get('HOME_URI') + '/api/ascents',
              method: 'POST',
              data: payload
            }, function (err, res) {
              console.log('result', err, res);
              tick.ascent = res.ascent._id;
            });
          }
          */
        });
