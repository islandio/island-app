...
exports.routes = function (app) {
  ...
  app.post('/api/posts', function (req, res) {
    ...
    db.Posts.create(props, {inflate: {author: profiles.member},
      ...
      // Index this post.
      if (doc.title && doc.title !== '' && doc.title.match(/\w+/g))
        search.index(doc.title, doc._id.toString());
      if (doc.author.displayName && doc.author.displayName !== ''
        && doc.author.displayName.match(/\w+/g))
        search.index(doc.author.displayName, doc._id.toString());
      if (doc.author.username && doc.author.username !== '')
        if (doc.author.username.match(/\w+/g))
          search.index(doc.author.username, doc._id.toString());
      ...
      Step(
        ...
        function (err, medias) {
          ...
          // Publish post.
          pubsub.publish('posts', 'post.new', doc);
          ...
          // Facebook...
          if (facebook)
            postToFacebook(doc, app.get('facebook'), function (err) {
              if (err && err instanceof ShareError) {
                ...
                pubsub.publish('mem-' + doc.author._id.toString(),
                    'flash.new', {
                  message: message,
                  level: 'error',
                  sticky: true
          ...
          // Twitter...
          if (twitter)
            tweetToTwitter(doc, app.get('twitter'), function (err) {
              if (err && err instanceof ShareError) {
                util.error(req.user.username + ': '
                    + JSON.stringify(err.message));
          ...
          Step(
            ...
            function (err, update) {
              ...
              _.each(medias, _.bind(function (m, i) {
                var q = "UPDATE " + app.get('cartodb').tables.medias
                ...
                curl.request({
                  url: 'https://' + app.get('cartodb').user
                      + '.cartodb.com/api/v2/sql',
                  method: 'POST',
                  data: {q: q, api_key: app.get('cartodb').api_key}
                }, this.parallel());
              }, this));
            },
            function (err) {
  ...
  // Mark as featured
  app.post('/api/posts/feature/:un/:k', function (req, res) {
    if (!req.user || !req.user.admin)
      return res.send(403, {error: 'Member invalid'});

    // Update the post.
    var key = [req.params.un, req.params.k].join('/');
    db.Posts.update({key: key}, {$set: {featured: true}}, function (err, stat) {
      if (com.error(err, req, res)) return;
      res.send({updated: true})      
    });
