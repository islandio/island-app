...
function postToFacebook(post, keys, cb) {
  ...
  if (parsed.caption !== '') {
    ...
    params.picture = parsed.images.length > 0 ?
        (parsed.images[0].image.ssl_url || parsed.images[0].image.cf_url):
        (parsed.videos.length > 0 ? (parsed.videos[0].image.ssl_url
        || parsed.videos[0].image.cf_url) : null);
  } else {
    if (post.title !== '') {
...
exports.routes = function (app) {
  ...
  app.post('/api/posts', function (req, res) {
    ...
    db.Posts.create(props, {inflate: {author: profiles.member},
      ...
      if (post.author.displayName && post.author.displayName !== ''
        && post.author.displayName.match(/\w+/g))
        search.index(post.author.displayName, post._id.toString());
      if (post.author.username && post.author.username !== '')
        if (post.author.username.match(/\w+/g))
          search.index(post.author.username, post._id.toString());
