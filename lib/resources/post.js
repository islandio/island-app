    ...
    Step(
      ...
      function (err, parent) {
        ...
        db.Posts.create(props, {inflate: {author: profiles.member},
          ...
          cache.index('posts', doc, ['title'], {strategy: 'noTokens'});
          ...
          Step(
            ...
            function (err, medias) {
              ...
              if (parent) {
                ...
                switch (type) {
                  case 'member':
                    ...
                    event.data.target.v = parent.avatar ? parent.avatar.ssl_url: undefined;
                    break;
                  case 'crag':
              ...
              if (facebook) {
                sharing.postToFacebook(req.user, doc.key, function (err) {
                  if (err && err instanceof sharing.ShareError) {
                    console.log('ShareError: ' + req.user.username + ': ' +
                    ...
                    events.publish('mem-' + req.user._id.toString(), 'flash.new', {
                      data: {
                        message: message,
                        level: 'error',
                        sticky: true
                    ...
                    console.log('Error in postToFacebook: ' + (err.stack || err));
                  }
                });
              }
              ...
              if (twitter) {
                ...
                sharing.tweet(req.user, doc.key, txt, function (err) {
                  if (err && err instanceof sharing.ShareError) {
                    console.log('ShareError: ' + req.user.username + ': ' +
                        JSON.stringify(err.message));
                    ...
                    console.log('Error in tweet: ' + (err.stack || err));
                  }
                });
              }
            }
