 ...
 * 
 * We want to let users post to their feed by tagging their (not others!)
...
exports.routes = function () {
  ...
  function _isIslandPost(p) {
    var txt = p.media.caption.text;
    if (txt.indexOf('@' + app.get('INSTAGRAM_USER')) !== -1) {
      return true;
    }
    return !!_.find(app.get('INSTAGRAM_TAGS').split(':'), function (tag) {
  ...
  app.post('/api/instagrams/users', function (req, res) {
    ...
    Step(
      ...
      function (err) {
        if (err) return this(err);
        posts = _.reject(posts, function (p) {
          return !p.media || !p.media.caption || !p.media.caption.text
              || !_isIslandPost(p);
        });
        if (posts.length === 0) {
          return this();
