...
exports.routes = function (app) {
  ...
  app.put('/api/sessions/:id', function (req, res) {
    ...
    /*
      ...
      - check new date
      - if diff
        - remove tick from action, session
        - find or create session w/ same date
        - change tick action, session
      - update other props
      - publish update event
      - change tick event details?
      - publish session if new
    */

    // Get the tick.
    db.Ticks.findOne({_id: db.oid(props.tick_id)}, function (err, tick) {
