...
exports.routes = function () {
  ...
  function publishTick(member, tick, session, ascent, crag) {
    Step(
      function () {
  ...
  app.post('/api/sessions', function (req, res) {
    ...
    if ((!req.body.crag_id && !req.body.cragName) || !req.body.date) {
      return res.send(403, {error: 'Session invalid'});
    }

    var props = _.isArray(req.body) ? req.body : req.body;
    ...
    // Get the crag.
    db.Crags.findOne({$or: [{_id: db.oid(props.crag_id)}, {name: props.cragName}]},
        function (err, crag) {
              ...
              function (err, a) {
                ...
                _.each(ticks, function (tick) {
                  ...
                  db.Ascents.findOne({$or: [{_id: db.oid(tick.ascent_id)},
                      {name: tick.ascentName, crag_id: crag._id}]},
                      function (err, ascent) {
