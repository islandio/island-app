  ...
  function publishTick(member, tick, session, ascent, crag, share) {
    Step(
      ...
      function (err, e) {
        ...
        if (share.facebook) {
          sharing.postToFacebook(member, sharePath, function (err) {
            if (err && err instanceof sharing.ShareError) {
              console.log('ShareError: ' + member.username + ': ' +
              ...
              events.publish('mem-' + member._id.toString(), 'flash.new', {
                data: {
                  message: message,
                  level: 'error',
                  sticky: true
        ...
        if (share.twitter) {
          var txt = tick.note || '';
          sharing.tweet(member, sharePath, txt, function (err) {
            if (err && err instanceof sharing.ShareError) {
              console.log('ShareError: ' + member.username + ': ' +
                  JSON.stringify(err.message));
        ...
        Step(
          function () {
            ...
            if (crag.location) {
              ...
              var ms = new Date(d.getFullYear(), d.getMonth(), d.getDate()).valueOf();
