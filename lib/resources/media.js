...
exports.routes = function (app) {
  ...
  app.post('/api/medias/:type', function (req, res) {
    if (!req.body.link || !req.body.parent_id)
    ...
    db[resource].read({_id: db.oid(props.parent_id)}, function (err, parent) {
      ...
      Step(
        function () {
          ...
          if (props.link.type === 'vimeo') {
            request.get({
              uri: 'https://vimeo.com/api/v2/video/' + props.link.vid + '.json',
              json: true
            }, _.bind(function (err, res, body) {
              if (err) return this(err);
              if (body.error) return this(body.error);
              props.thumb_uri = body[0].thumbnail_small;
              props.poster_uri = body[0].thumbnail_large;
              this();
            }, this));
          } else if (props.link.type === 'youtube') {
            props.thumb_uri = 'https://img.youtube.com/vi/' + props.link.vid + '/2.jpg';
            props.poster_uri = 'https://img.youtube.com/vi/' + props.link.vid + '/0.jpg';
            this();
          } else this();
        },
        function (err) {
          ...
          db.Medias.create(props, {inflate: {author: profiles.member}},
            ...
            Step(
              function () {
                if (!media.thumb_uri || !parent.location) return this();
              ...
              function (err) {
                ...
                // Publish media.
                pubsub.publish(type + '-' + parent._id.toString(),
                    'media.new', media);
