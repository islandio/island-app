...
exports.routes = function () {
  ...
  function createAscent (props, cb) {
    if (!props.crag_id || !props.name) {
      return cb('Ascent invalid');
    }
    var user = props.user;
    if (user) props.author_id = user._id;
    delete props.user;
        ...
        db.Ascents.create(_.extend(props, {
          ...
          Step(
            ...
            function (err) {
              if (err) return cb(err);
              if (user) {
                publishAscent(user, ascent, this);
              } else {
                return this(null);
              }
            },
  ...
  app.post('/api/ascents', function (req, res) {
    ...
    createAscent(req.body, function(err, doc) {
