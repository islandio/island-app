...
var calculateGradeByConsensus = exports.calculateGradeByConsensus =
    function(consensus) {
...
exports.routes = function () {
  ...
  exports.removeConsensus =
      function(ascent, member, cb) {
  ...
  exports.updateConsensus =
      function(grade, ascent, member, tick, cb) {
  ...
  var moveAscent = exports.moveAscent = function (member, id, cragId, cb) {
    if (!member) {
  ...
  var mergeAscents = exports.mergeAscents = function (member, ids, props, cb) {
    if (!member) {
      return cb({code: 403, error: {message: 'Member invalid'}});
    }
    ...
    db.Ascents.read({_id: ids.shift()}, function (err, leader) {
      if (err) return cb(err);
      if (!leader) {
        return cb({error: {code: 404, message: 'Ascent not found'}});
      }
      ...
      var consensus = _.reject(leader.consensus, function (c) {
        return !c.author_id && !c.tick_id;
      });
      consensus.unshift({grade: Number(props.grade)});
      var update = {
        name: props.name,
        rock: props.rock,
        type: props.type,
  ...
  app.put('/api/ascents/:id', function (req, res) {
    ...
    db.Ascents.read({_id: db.oid(req.params.id)}, function (err, ascent) {
      ...
      if (props.grade) {
        var consensus = ascent.consensus;
        // Remove inital grade suggestion
        consensus.shift();
  ...
  app.post('/api/ascents/move', function (req, res) {
    ...
    Step(
      function () {
        _.each(req.body.ascent_ids, _.bind(function (id) {
          moveAscent(req.user, id, req.body.crag_id, this.parallel());
        }, this));
      },
      function (err) {
        if (errorHandler(err, req, res)) return;
  ...
  app.post('/api/ascents/merge', function (req, res) {
    ...
    mergeAscents(req.user, req.body.ascent_ids, req.body.props,
        function (err) {
      if (errorHandler(err, req, res)) return;

      res.send({merged: true});
    });
  });
