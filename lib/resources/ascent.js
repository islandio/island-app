...
var refreshAscentCounts = require('../resources/crag').refreshAscentCounts;
var app = require('../../app');
...
var calculateGradeByConsensus =
  ...
  var f = _.find(consensus, function(c) {
...
exports.routes = function () {
    ...
    db.Ascents.read({_id: ids[0]}, function (err, leader) {
      ...
      Step(
        ...
        function (err, merged) {
          ...
          // Re-map the ascent's crag.
          refreshAscentCounts(merged.crag_id, function (err) {
  ...
  var reKeyAscent = exports.reKeyAscent = function (id, member, cb) {
    ...
    db.Ascents.read({_id: id}, function (err, ascent) {
      ...
      db.Crags.read({_id: ascent.crag_id}, function (err, crag) {
        ...
        var key = [crag.key, type, _s.slugify(ascent.name)].join('/');
        var update = {key: key};
  ...
  var deleteAscent = exports.deleteAscent = function (id, member, cb) {
    ...
    function _deleteResource(type, cb) {
      db[_s.capitalize(type) + 's'].list({$or: [{ascent_id: id},
        ...
        Step(
          ...
          function (err) {
            ...
            db[_s.capitalize(type) + 's'].remove({$or: [{ascent_id: id},
                {parent_id: id}]}, this);
          }, cb
        );
      });
      ...
      Step(
        ...
        function (err, crag) {
          ...
          db.Ascents.create(props, {force: {key: 1}}, function (err, ascent) {
            ...
            refreshAscentCounts(ascent.crag_id);
  ...
  app.put('/api/ascents/:id', function (req, res) {
    ...
    if (_.isEmpty(props) && _.isEmpty(unset)) {
      return res.status(403).send({error: {message: 'Ascent empty'}});
    }
  ...
  app.delete('/api/ascents/:id', function (req, res) {
    ...
    // Get crag so we can update its ascent count.
    db.Ascents.read({_id: db.oid(req.params.id)}, function (err, ascent) {
      if (errorHandler(err, req, res, ascent, 'ascent')) return;

      Step(
        function () {
          deleteAscent(req.params.id, req.user, this);
        },
        function (err) {
          if (err) return this(err);

          refreshAscentCounts(ascent.crag_id, this);
        },
        function (err) {
          if (errorHandler(err, req, res)) return;
  ...
  app.delete('/api/ascents', function (req, res) {
    ...
    // Get crag from one of the ascents so we can update its ascent count.
    db.Ascents.read({_id: db.oid(req.body.ascent_ids[0])}, function (err, ascent) {
      if (errorHandler(err, req, res, ascent, 'ascent')) return;
    ...
    db.Ascents.read({_id: db.oid(req.body.ascent_ids[0])}, function (err, ascent) {
      ...
      Step(
        ...
        function (err) {
          ...
          refreshAscentCounts(req.body.crag_id, this.parallel());
          refreshAscentCounts(ascent.crag_id, this.parallel());
        },
        function (err) {
          if (errorHandler(err, req, res)) return;
