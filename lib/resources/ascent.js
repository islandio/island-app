...
exports.routes = function () {
  ...
  app.put('/api/ascents/:id', function (req, res) {
    ...
    db.Ascents.read({_id: db.oid(req.params.id)}, function (err, ascent) {
      ...
      if (props.grade) {
        update.$pull = {consensus: {author_id: req.user._id }};
      }
      ...
      db.Ascents.update({_id: ascent._id}, update, function (err, stat) {
        ...
        // Update grade conensus
        if (props.grade) {
          var push = {};
          push.$push = {consensus: {
            grade: props.grade, author_id: req.user._id }};
          db.Ascents.update({_id: ascent._id}, push, function (err, stat) {
            if (errorHandler(err, req, res, stat, 'ascent')) return;
            finish();
          });
        } else {
