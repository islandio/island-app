...
exports.routes = function () {
  ...
  app.post('/api/hangtens/:type', function (req, res) {
    if (!req.body.parent_id) {
      return res.status(403).send({error: 'Hangten invalid'});
    }
    if (!req.user) {
    ...
    var typeResource = _s.capitalize(type) + 's';
            ...
            switch (type) {
              case 'post':
                target.n = parent.title !== '' ?
                    parent.title: _s.prune(parent.body, 20);
                target.s = parent.key;
                break;
              case 'tick':
              ...
              case 'crag':
                ...
                if (parent.parent) {
                  target.pn = parent.parent.name;
                  target.pl = parent.parent.country;
                  target.ps = ['crags', parent.parent.key].join('/');
                }
                break;
              case 'ascent':
                target.n = parent.name;
