  ...
  Step(
    ...
    function (err, member) {
      ...
      req.login(member, function (err) {
        if (err) return cb(err);
        var referer = getReferer(req.session.referer);
    ...
    Step(
      function () {
        ...
        createMember({
          ...
          displayName: 'Island',
          invited: new Date(),
          email: 'admin@island.io',
          password: 'weare1s1and'
        }, true, this);
      },
      function (err, island) {
        if (err) return this(err);
...
exports.routes = function () {
  ...
  app.get('/connect/twitter', function (req, res, next) {
    ...
    var returnUrl = url.format(referer);
    passport._strategies['twitter-authz']._oauth._authorize_callback =
        returnUrl;
  ...
  app.put('/api/members/:un', function (req, res) {
    if (!req.user || req.user.username !== req.params.un) {
      return res.status(403).send({error: {message: 'Member invalid'}});
    }
    ...
    if (props.username !== undefined && props.username.length < 4) {
      return res.status(403).send({error: {message: 'Username too short'}});
    }

    // Check blacklist.
    if (props.username && _.contains(BLACKLIST, props.username)) {
      return res.status(403).send({error: {message: 'Username exists'}});
    }

    // Ensure displayName is not empty.
    if (props.displayName !== undefined && props.displayName.length < 4) {
      return res.status(403).send({error: {message: 'Name too short'}});
    }
    ...
    db.Members.update({username: req.params.un}, update,
      ...
      if (err && err.code === 11000) {
        if (err.err.indexOf('username') !== -1) {
          ...
          return res.status(403).send({error: {message: 'Email address exists'}});
        } else {
          return res.status(403).send({error: {message: 'Unknown duplicate'}});
        }
      }
      if (errorHandler(err, req, res, stat, 'member')) return;
  ...
  */
  app.delete('/api/members/:un', function (req, res) {
    ...
    function _deleteResource(type, cb) {
      db[_s.capitalize(type) + 's'].list({author_id: mid}, function (err, docs) {
        if (err) return cb(err);
        if (docs.length === 0) {
          return cb();
        }
        ...
        Step(
          ...
          function (err) {
            ...
            db[_s.capitalize(type) + 's'].remove({author_id: mid}, this);
          }, cb
        );
      });
    }
    ...
    Step(
      function () {
        ...
        // Get ticks for consensus handling.
        db.Ticks.list({author_id: mid}, this);
      },
      function (err, ticks) {
        if (err) return this(err);
        if (ticks.length === 0) {
        ...
        _.each(ticks, function (t) {
          // Remove tick's consensus contribution.
          Ascents.removeConsensus(t.ascent_id, t.author_id, parallel());
        });
      },
      function (err) {
        if (err) return this(err);
      ...
      function (err) {
        ...
        events.publish('member', 'member.removed', {data: {
            id: mid.toString()}});
          ...
          if (!pass) {
            Step(
              ...
              function (err, signup) {
                if (errorHandler(err, req, res)) return;
                if (!signup) {
                  res.status(401).send({error: {message: 'Hey, comrade! Please ' +
                      '<a href="/">request an invite</a> at ' +
                      member.primaryEmail +
                      ' so we can send you one when the time comes.',
                      level: 'alert'}});
                } else {
                  res.status(401).send({error: {message:
                      'Hey, comrade! We\'ll send you an invite soon.',
                      level: 'alert'}});
                }
              }
            );
  ...
  app.post('/api/members/forgot', function (req, res) {
    ...
    db.Members.read({primaryEmail: req.body.email}, function (err, member) {
      ...
      if (!member.password) {
        return res.status(403).send({error: {message: 'No password'},
  ...
  app.post('/api/members/reset', function (req, res) {
    ...
    if (req.body.newpassword.length < 7) {
      return res.status(403).send({error: {message: 'Password too short'}});
    }

    // Compare new passwords.
    if (req.body.newpassword !== req.body.cnewpassword) {
      return res.status(403).send({error: {message: 'Passwords do not match'}});
    }
    ...
    if (req.user) {
      ...
      db.Members.read({_id: req.user._id}, function (err, member) {
        ...
        if (!member.password || !req.body.oldpassword ||
            iutil.encrypt(req.body.oldpassword, member.salt) !==
            member.password) {
          return res.status(401).send({error: {message: 'Invalid credentials'}});
        } else {
          _update(member._id);
    ...
    // Check for session token.
      ...
      return res.status(403).send({error: {
          message: 'Password reset session invalid'}});
    }
