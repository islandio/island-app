...
exports.init = function (app) {
  ...
  passport.use('instagram-authz', new InstagramStrategy(
      _.extend(app.get('instagram'), {callbackURL: app.get('HOME_URI')
      + app.get('instagram').callbackURL}),
    function (token, refresh, profile, cb) {
...
exports.routes = function (app) {
  ...
  app.post('/api/members/:un/unfollow', function (req, res) {
    ...
    db.Members.read({username: req.params.un}, function (err, mem) {
      ...
      // Remove subscription.
      pubsub.unsubscribe(req.user, mem, function (err, stat) {
        if (com.error(err, req, res, stat, 'subscription')) return;
    ...
    db.Members.list({instagramId: {$exists: true}}, function (err, members) {
      ...
      Step(
        ...
        function (err, grams) {
          ...
          var query = "INSERT INTO " + app.get('cartodb').tables.instagrams + " ("
              + _.join(",", names) + ") VALUES ";
