 ...
 */
function subscribeToIsland(db, member, cb) {
  db.Members.read({username: 'island'}, function (err, island) {
    if (err) return cb(err);
    if (!island) {
      return cb('Could not find the island user');
    }

    // Create subscription.
    app.get('events').subscribe(member, library, {style: 'follow',
 ...
 * Create local member.
 */
var createMember = exports.createMember = function(memberProps, cb) {
  var db = app.get('db');
  ...
  // Check details.
  memberProps.username = _.slugify(memberProps.username).substr(0, 30);
  ...
  // Check blacklist.
  if (_.contains(BLACKLIST, memberProps.username)) {
  ...
  var props = {
    ...
    displayName: memberProps.username,
  ...
  db.Members.create(props, function (err, member) {
    ...
    // Sub member to library.
    subscribeToLibrary(db, member, function (err) {
      cb(err, member);
    });
  });
};
      ...
      if (!member) {
        ...
        if (!props.username) {
          props.username = props.displayName && props.displayName !== '' ?
              _.slugify(props.displayName): iutil.key();
        }
        ...
        db.Members.create(props, {force: {username: 1}},
          ...
          subscribeToIsland(db, member, function (err) {
            cb(err, member);
          });
        });
        return;
  ...
  */
  ...
  app.post('/api/members/search/:s', function (req, res) {
    Step(
      ...
      // Perform the search.
      function () {
        cache.search('members', req.params.s, 20, this);
      },
