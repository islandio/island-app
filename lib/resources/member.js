  ...
  passport.use(new FacebookStrategy(app.get('facebook'),
    ...
    db.Members.read({facebookId: props.id}, function (err, member) {
      ...
      if (!member) {
        ...
        // Create a new member.
        return db.Members.create(props, {force: {username: 1}}, cb);
      }
...
exports.routes = function (app) {
  ...
  app.put('/api/members/:un', function (req, res) {
    ...
    // Check details.
    var props = req.body;
    if (props.username && props.username.length < 4)
      return res.send(403, {error: 'Username too short'});
        ...
        Step(
          ...
          function (err) {
            ...
            _.each(posts, function (p) {
              ...
              db.Views.remove({parent_id: p._id}, parallel());
            ...
            db.Views.remove({author_id: mid}, parallel());
  ...
  app.post('/api/members/auth', function (req, res) {
    ...
    // Find member.
    db.Members.read({username: req.body.username}, function (err, member) {
      if (com.error(err, req, res)) return;
    ...
    search.query(req.params.s).end(function (err, ids) {
      ...
      Step(
        ...
        function (err, mems) {
          ...
          // Send profile.
          res.send(com.client({items: mems || []}));
