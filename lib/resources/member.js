        ...
        Step(
          ...
          function (err) {
            ...
            _.each(medias, function (m) {
              ...
              // Publish removed status.
              pubsub.publish('ascent-' + m.parent_id.toString(),
                  'media.removed', {id: m._id.toString()});
  ...
  app.post('/api/members/:un/follow', function (req, res) {
    ...
    db.Members.read({username: req.params.un}, function (err, mem) {
      ...
      db.Subscriptions.create({
        ...
        subscribee_id: mem._id,
        meta: {style: 'follow', type: 'member'},
        mute: false
      }, function (err, sub) {
        if (err && err.code !== 11000 && com.error(err, req, res)) return;
