...
/* Check if this Email address was invited into the Beta. Stub me out later */
function checkBeta(member, cb) {
  var db = app.get('db');
  if (member && member.admin) return cb(null, true);
  // Make sure there is at least one admin user who can approve a beta signup
  db.Members.findOne({admin: true}, function(err, mem) {
    if (err) return cb(err);
    if (!mem) return cb(null, true);
    else {
      db.Signups.findOne({email: member.primaryEmail}, function(err, signup) {
        if (err) return cb(err);
        return cb(null, signup && signup.invited ? true : false);
      });
    }
  });
}
...
var createMember = exports.createMember = function(memberProps, force, cb) {
  ...
  checkBeta(props, function(err, beta) {
    if (err || !beta) return cb({message: "We haven't invited you yet, but soon!"});
...
exports.routes = function () {
  ...
  app.get('/auth/facebook/callback', function (req, res, next) {
    passport.authenticate('facebook', function (err, member, info) {
      ...
      checkBeta(member, function(err, beta) {
        if (err || !beta) return next('We haven\'t invited you yet');
        // Login.
  ...
  app.post('/api/members', function (req, res) {
    createMember(req.body, function(err, member) {
      if (err) {
        ...
        checkBeta(member, function(err, beta) {
          if (err || !beta)
            res.send(403, {error: 'We haven\'t invited you yet'});
          req.login(member, function (err) {
            if (errorHandler(err, req, res)) return;
