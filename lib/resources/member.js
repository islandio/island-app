 ...
 */
function indexMember(user, cache) {
  cache.index('members', user, ['displayName', 'username'], null);
}
...
exports.routes = function (app) {
  ...
  app.post('/api/members', function (req, res) {
    ...
    var props = {
      ...
      displayName: req.body.newdisplayname || req.body.newusername,
  ...
  app.delete('/api/members/:un', function (req, res) {
    ...
    Step(
      function () {
        ...
        // Get member's own posts, medias, and events.
        db.Posts.list({author_id: mid}, this.parallel());
        db.Medias.list({author_id: mid}, this.parallel());
        db.Events.list({actor_id: mid}, this.parallel());
      },
      function (err, posts, medias, events) {
        if (com.error(err, req, res)) return;
        ...
        Step(
          ...
          function () {
            ...
            // Remove notifications for events where member's medias are target or action.
            if (medias.length === 0) return this();
            var next = _.after(medias.length, this);
            _.each(medias, function (m) {
              db.Events.list({$or: [{target_id: m._id}, {action_id: m._id}]},
                  function (err, events) {
                if (events.length === 0) return next();
                var _next = _.after(events.length, next);
                _.each(events, function (e) {
          ...
          function (err) {
            ...
            _.each(posts, function (p) {
              ...
              // Publish removed status.
              pubsub.publish('post', 'post.removed', {data: {id: p._id.toString()}});
            ...
            // Remove others' content on member's medias.
            _.each(medias, function (m) {

              // Publish removed status.
              pubsub.publish('media', 'media.removed', {data: {id: m._id.toString()}});

              db.Comments.remove({parent_id: m._id}, parallel());
              db.Subscriptions.remove({subscribee_id: m._id}, parallel());
              db.Events.remove({$or: [{target_id: m._id}, {action_id: m._id}]},
                  parallel());
            });
  ...
  app.post('/api/members/:un/follow', function (req, res) {
    ...
    db.Members.read({username: req.params.un}, function (err, mem) {
      ...
      // Create subscription.
      pubsub.subscribe(req.user, mem, {style: style, type: 'member'},
          function (err, sub) {
  ...
  app.post('/api/members/:un/unfollow', function (req, res) {
    ...
    db.Members.read({username: req.params.un}, function (err, mem) {
      ...
      // Remove subscription.
      pubsub.unsubscribe(req.user, mem, function (err, sub) {
        if (com.error(err, req, res, sub, 'subscription')) return;
  ...
  app.post('/api/members/search/:s', function (req, res) {
    ...
    // Perform the search.
    cache.search('members', req.params.s, 20, function (err, ids) {
    ...
    db.Members.list({instagramId: {$exists: true}}, function (err, members) {
      ...
      Step(
        ...
        function (err, grams) {
          ...
          var query = "INSERT INTO " + app.get('CARTODB_INSTAGRAM_TABLE') + " ("
              + _.join(",", names) + ") VALUES ";
        ...
        function (err, data) {
          ...
          if (data) {
            ...
            // Publish.
            pubsub.publish('map', 'instagram.new', {data: {}});
          }
        }
      );
