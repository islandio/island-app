...
function checkBeta(email, cb) {
  ...
  db.Members.findOne({admin: true}, function(err, mem) {
    ...
    if (!mem) return cb(null, true);
      ...
      db.Signups.findOne({email: email}, function(err, signup) {
        if (err) return cb(err);
...
var createMember = exports.createMember = function(memberProps, force, cb) {
  ...
  checkBeta(memberProps.email, function(err, beta) {
  ...
  passport.use(new FacebookStrategy({
    ...
    db.Members.read(query, function (err, member) {
      ...
      if (!member) {
        ...
        // Create a new member.
        checkBeta(props.primaryEmail, function(err, beta) {
          if (!beta) return;
