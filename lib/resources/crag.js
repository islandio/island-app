...
exports.routes = function (app) {
  ...
  // Create
  // TODO: 
  //   - map new crag to cartodb
  //   - create event ?
  //     - create event is first event in crag's feed
  //   - auto-watch crag for creator
    ...
    Step(
      function () {
        ...
        var codeUrl = 'http://api.geonames.org/countryCodeJSON?formatted=true'
            + '&username=islandio&lat=' + props.lat + '&lng=' + props.lon;
      ...
      function (err, country) {
        ...
        db.Crags.create(_.extend(props, {
          ...
          Step(
            ...
            function (err) {
              if (com.error(err, req, res)) return;
              res.send({added: true, country: crag.country});
            }
          );
        });
      }
