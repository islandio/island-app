...
var mapRequest = exports.mapRequest = function (sql, cb) {
  curl.request({
...
var mapCrag = exports.mapCrag = function (crag, cb) {
  var names = [
    "the_geom",
    "id",
    "name",
...
var unMapCrag = exports.unMapCrag = function (id, cb) {
  mapRequest("DELETE FROM " + app.get('CARTODB_CRAGS_TABLE') + " WHERE " +
       "id = '" + id.toString() + "'", cb);
};

exports.refreshAscentCounts = function (id, cb) {
  var db = app.get('db');
  ...
  Step(
    function () {
      db.Crags.read({_id: id}, this.parallel());
      db.Ascents.count({crag_id: id, type: 'b'}, this.parallel());
      db.Ascents.count({crag_id: id, type: 'r'}, this.parallel());
    },
    function (err, crag, bcnt, rcnt) {
      if (err) return this(err);
      db.Crags.update({_id: crag._id}, {$set: {bcnt: bcnt, rcnt: rcnt}},
          this.parallel());
      mapCrag(crag, this.parallel());
    },
    function (err) {
      if (cb) {
        cb(err);
...
exports.routes = function () {
  ...
  function publishCrag(mem, crag, cb) {
    Step(
      function () {
        db.inflate(crag, {author: profiles.member, parent: profiles.crag},
            this.parallel());
      ...
      function (err) {
        ...
        events.publish('crag', 'crag.new', {
          ...
          event: {
            actor_id: mem._id,
            target_id: crag.parent ? crag.parent._id: null,
            action_id: crag._id,
            action_type: 'crag',
            data: {
              action: {
  ...
  function indexCrag(c) {
    console.log("index crag")
    cache.index('crags', c, ['name', 'country']);
  }
  ...
  app.post('/api/crags', function (req, res) {
    if (!req.body.name || !req.body.location) {
      return res.status(403).send({error: {message: 'Crag invalid'}});
    }
    var props = req.body;
    var countryInfo;
    var countryKey;
    ...
    Step(
      function () {
        getCountryAtLocation(props.location, this.parallel());

        if (props.parent_id) {
          props.parent_id = db.oid(props.parent_id);
          db.Crags.read({_id: props.parent_id}, this.parallel());
        } else {
          delete props.parent_id;
        }
      },
      function (err, country, parent) {
        console.log(err, country, parent)
        if (errorHandler(err, req, res, country, 'country')) return;
        if (props.parent_id &&
            errorHandler(err, req, res, parent, 'crag')) return;

        db.Crags.create(_.extend(props, {
          author_id: req.user ? req.user._id : null,
          country_id: country._id,
          country: country.name,
          key: [country.key, _s.slugify(props.name)].join('/'),
          bcnt: 0,
          rcnt: 0,
          verified: false,
          public: pub
        }), {force: {key: 1}}, function (err, crag) {
          console.log(err, crag, "down here")
  ...
  app.put('/api/crags/:country/:name', function (req, res) {
    ...
    if (props.name !== undefined && props.name.length < 3) {
      return res.status(403).send({error: {type: 'LENGTH_INVALID', message:
    ...
    if (_.isEmpty(props) && _.isEmpty(unset)) {
      return res.status(403).send({error: {message: 'Crag empty'}});
    }
  ...
  app.delete('/api/crags/:country/:name', function (req, res) {
    ...
    function _deleteResource(type, cb) {
      db[_s.capitalize(type) + 's'].list({$or: [{crag_id: cid},
        ...
        Step(
          ...
          function (err) {
            ...
            db[_s.capitalize(type) + 's'].remove({$or: [{crag_id: cid},
                {parent_id: cid}]}, this);
          }, cb
        );
      });
