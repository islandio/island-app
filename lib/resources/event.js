        ...
        switch (e.action_type) {
          ...
          case 'session':
            ...
            Step(
              ...
              // Get media associated with these ticks
              function (err) {
                if (err) return this(err);

                if (!e.action.actions) return _this();

                var docs = 0;
                _.each(e.action.actions, function(a) {
                  docs += a.ticks_cnt;
                });

                if (docs === 0) return _this();
                var next = _.after(docs, _this);
                _.each(e.action.actions, function(a) {
                  _.each(a.ticks, function (t) {
                    db.fill(t, 'Medias', 'parent_id', { sort: { created: -1 }}, next);
                  });
                });
              }
            );
