...
exports.feed = function (query, actions, options, cb) {
  ...
  function _finish(err, es) {
    ...
    es = _.reject(es, function (e) {
      if (app.get('package').beta && !e.action.author.invited)
        return true;
      delete e._reject;
      if (query.media) {
  ...
  function modifyTickQuery(query, cb) {
    ...
    if (query.tries && typeof query.tries === 'string') {
      if (query.tries === 'onsite') query.tries = 1;
      else if (query.tries === 'flash') query.tries = 2;
      else if (query.tries === 'redpoint') query.tries = {'$gte': 3};
    }
    if (query.grade && typeof query.grade === 'object'
      ...
      // FIXME change way grades are stored for ascents
      var ascentGrades = gradeConverter[query.grade.type].to('default')
          .indexes(gradeIdx);
      delete query.grade;
      ...
      Step (
        ...
        function(err, ticks) {
          ...
          var ascentIds = _.chain(ticks)
              .filter(function(t) {
                var found =  _.reduce(ascentGrades, function(m, g) {
                    var n = m || (t.ascent.grades.indexOf(g) !== -1);
                    return n;
                  }, false);
          ...
          query['$or'] = [{ascent_id: ascentQuery}, {grade: gradeQuery}];
          return cb();
        }
      )
    } else {
