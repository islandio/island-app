...
exports.routes = function (app) {
  ...
  // list
  app.post('/api/notifications/:un', function (req, res) {
    ...
    db.Members.read({username: req.params.un}, function (err, mem) {
      ...
      // db.Notifications.list({member_id: mem._id}, {sort: {created: -1},
      db.Notifications.list({}, {sort: {created: -1},
          limit: limit, skip: limit * cursor,
          inflate: {member: profiles.member}}, function (err, notes) {
  ...
  app.put('/api/notifications/read/:id', function (req, res) {
    ...
    db.Notifications.update({_id: db.oid(req.params.id)}, {$set: {read: true}},
        function (err, stat) {
