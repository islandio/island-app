...
exports.routes = function (app) {
  ...
  app.delete('/api/notifications/:id', function (req, res) {
    ...
    db.Notifications.delete({_id: db.oid(req.params.id)}, function (err, stat) {
      if (com.error(err, req, res, stat, 'notification')) return;
    ...
    //     // Get note.
    //     db.Notifications.read({_id: db.oid(req.params.id),
    //         subscriber_id: req.user._id}, this);
    //   },
    //   function (err, doc) {
    //     if (com.error(err, req, res, doc, 'notification')) return;

    //     // Delete the note.
    //     db.Notifications.delete({_id: doc._id}, this.parallel());

    //     // Get sub for this note.
    //     db.Subscriptions.read({_id: doc.subscription_id}, this.parallel());
    //   },
    //   function (err, stat, sub) {
    //     if (com.error(err, req, res, stat, 'notification')) return;
    //     if (com.error(err, req, res, sub, 'subscription')) return;

    //     // Remove subscription if it's a request.
    //     if (sub.meta.style === 'request')
    //       pubsub.unsubscribe(sub.subscriber_id, sub.subscribee_id, this);
    //     else this(null, true);
    //   },
    //   function (err, stat) {
