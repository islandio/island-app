  ...
  app.get('/service/home.profile', function (req, res) {
    ...
    Step(
      function () {
        ...
        if (req.user)
          // db.Notifications.list({member_id: req.user._id},
          db.Notifications.list({}, {sort: {created: -1}, limit: 10,
              inflate: {member: profiles.member}}, this.parallel());
      ...
      Step(
        function () {
          db.Comments.list({member_id: mem._id}, {sort: {created: -1},
              limit: 10, inflate: {member: profiles.member,
              post: profiles.post}}, this.parallel());
          db.Posts.list({member_id: mem._id}, {sort: {created: -1},
              limit: 5, inflate: {member: profiles.member}},
              this.parallel());
        },
