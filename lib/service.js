          ...
          Step(
            function () {
              ...
              if (allow) {
                ...
                var query = {action: {type: 'tick', query: {author_id: mem._id,
                    sent: true}}};
                Events.feed(query, ['tick'], {sort: {date: -1}}, this.parallel());

                // Events.feed({all: true, subscribee_id: mem._id}, ['tick'],
                //     {limit: limit, cursor: cursor}, this.parallel());
            ...
            /*
            function (err, ticks, feed, sidebar, notes) {
            */
