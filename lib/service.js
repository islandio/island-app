...
exports.routes = function () {
  ...
  function betaGate(req, res, member) {
    ...
    if (gate && req.xhr) {
      ...
      res.status(401).send(iutil.client({
        member: req.user,
        content: {page: null},
        root: app.get('ROOT_URI'),
        error: {
  ...
  function renderStatic(req, res, props, template) {
    ...
    if (props.description !== '') {
      ...
      if (len > 140) {
        var over = len - 140 + 3;
        props.shortDescription = _s.prune(props.description,
  ...
  // Static profile
  app.get('/service/about', function (req, res) {
    Step(
      function () {

        // List Ambassadors.
        db.Members.list({username: {$in: [
          'b3bouldering',
          'danielwoods',
          'nalle_hukkataival',
        ]}}, this.parallel());
      ...
      function (err, _ambassadors, notes) {
        if (errorHandler(err, req, res)) return;

        var ambassadors = []
        if (_ambassadors.length > 0) {
          _.each(_ambassadors, function(m) {
            var a = {};
            a._id = m._id;
            a.username = m.username;
            a.displayName = m.displayName;
            a.description = m.description;
            a.gravatar = iutil.hash(m.primaryEmail || 'foo@bar.baz');
            a.avatar = m.avatar ? m.avatar.ssl_url: undefined;
            a.avatar_big = m.avatar_big ? m.avatar_big.ssl_url: undefined;
            var index;
            switch (a.username) {
              case 'nalle_hukkataival':
                index = 0;
                break;
              case 'danielwoods':
                index = 1;
              ...
              case 'b3bouldering':
                index = 2;
                a.avatar_big = 'https://s3.amazonaws.com/island.io/b3bouldering.jpg'
                break;
            }
            ambassadors[index] = a
          });
        ...
        var profile = {
          member: req.user,
          transloadit: transloadit(req),
          content: _.extend({page: {ambassadors: ambassadors}})
        };
        if (notes) {
          profile.notes = {
            cursor: 1,
  ...
  // Activity profile
  app.get('/service/activity', function (req, res) {
    ...
    Step(
      ...
      function (err, mem) {
        ...
        if (req.user) {
          if (pub) {
            Events.feed({member_id: req.user._id, all: true},
        ...
        function (err, allow) {
          ...
          Step(
            ...
            function (err, ticks, feed, notes) {
              ...
              _.each(ticks, function (t) {
                var _t = {};
                _t.ascent = {name: t.ascent.name};
                _t.crag = {name: t.crag.name,
                    country: t.crag.country};
                _t.grade = t.grade ? t.grade : t.ascent.grade;
                _t.type = t.type;
                _t.date = t.date;
                _t.tries = t.tries;
                _t._id = t._id;
    ...
    db.Posts.read({key: key}, {inflate: {author: profiles.member}, inc: true},
      ...
      Step(
        ...
        function (err) {
          ...
          // Get product featured athletes.
          if (post.product && post.product.featured) {
            db.Members.list({_id: {$in: post.product.featured}},
                this.parallel());
          } else {
            this.parallel()(null, []);
          }
        ...
        function (err, sub, _featured, notes) {
          if (errorHandler(err, req, res)) return;

          if (_featured.length > 0) {
            var featured = [];
            _.each(_featured, function(m) {
              var f = {};
              f._id = m._id;
              f.username = m.username;
              f.displayName = m.displayName;
              f.description = m.description;
              f.gravatar = iutil.hash(m.primaryEmail || 'foo@bar.baz');
              f.avatar = m.avatar ? m.avatar.ssl_url: undefined;
              f.avatar_big = m.avatar_big ? m.avatar_big.ssl_url: undefined;
              featured.push(f)
            });
            post.product.featured = featured;
          }
    ...
    db.Crags.read({key: key, forbidden: {$ne: true}}, function (err, crag) {
      ...
      Step(
        function () {
          ...
          // Get sectors.
          db.Crags.list({parent_id: crag._id}, this.parallel());
        ...
        function (err, weather, feed, ascents, watchers, sectors, notes) {
          if (errorHandler(err, req, res)) return;

          // Filter ascents by grade.
          crag.ascents = {b: {}, r: {}, bcnt: 0, rcnt: 0};
          _.each(ascents, function (a) {
            if (crag.ascents[a.type][a.grade]) {
              crag.ascents[a.type][a.grade].push(a);
            } else {
              crag.ascents[a.type][a.grade] = [a];
            }
          ...
          if (weather) {
            try {
              ...
              console.log('Error in parse weather (' + weather.body + '): ' +
  ...
  app.get('/service/crag/:y/:g/config', function (req, res) {
    if (betaGate(req, res)) return;
    if (!req.user) {
      return res.status(403).send(iutil.client({
        content: {page: null},
        transloadit: transloadit(req),
  ...
  app.get('/service/media', function (req, res) {
    ...
    Step(
      function () {
        Events.feed({member_id: req.user ? req.user._id: null, all: true,
            media: true}, ['post', 'tick'], {limit: limit, cursor: cursor},
            this.parallel());
  ...
  poet.addRoute('/blog/category/:category', function (req, res) {
    ...
    getBlogPostsWithCategory(cat, function (err, posts) {
      ...
      renderStatic(req, res, {
        ...
        blog: true,
        iutil: iutil,
        title: 'Island | Blog - ' + _s.capitalize(cat),
        og_title: 'Island Blog - ' + _s.capitalize(cat),
  ...
  poet.addRoute('/blog/page/:page', function (req, res) {
    ...
    getBlogPosts(n * page - n, n * page, function (err, posts) {
      ...
      renderStatic(req, res, {
        ...
        blog: true,
        iutil: iutil,
        title: 'Island | Blog - page ' + page,
        og_title: 'Island Blog - page ' + page,
        description: 'News and feature updates.'
      }, 'blog/page');
    });
  });
    ...
    db.Countries.read({key: key}, function (err, country) {
      ...
      Step(
        ...
        function (err, bcnt, rcnt, efforts, sends, imgURL) {
          ...
          s1 = _s.numberFormat(bcnt, 0) + ' problem';
          if (bcnt !== 1) {
            s1 += 's';
          }

          s2 = _s.numberFormat(rcnt, 0) + ' route';
          if (rcnt !== 1) {
            s2 += 's';
          }

          s3 = _s.numberFormat(efforts, 0) + ' effort';
          if (efforts !== 1) {
            s3 += 's';
          }

          s4 = _s.numberFormat(sends, 0) + ' ascent';
          if (sends !== 1) {
            s4 += 's';
          }
    ...
    db.Crags.read({key: key}, function (err, crag) {
      ...
      Step(
        ...
        function (err, watchers, efforts, sends, imgURL) {
          ...
          s1 = _s.numberFormat(crag.bcnt, 0) + ' problem';
          if (crag.bcnt !== 1) {
            s1 += 's';
          }

          s2 = _s.numberFormat(crag.rcnt, 0) + ' route';
          if (crag.rcnt !== 1) {
            s2 += 's';
          }

          s3 = _s.numberFormat(watchers, 0) + ' watcher';
          if (watchers !== 1) {
            s3 += 's';
          }

          s4 = _s.numberFormat(efforts, 0) + ' effort';
          if (efforts !== 1) {
            s4 += 's';
          }

          s5 = _s.numberFormat(sends, 0) + ' ascent';
          if (sends !== 1) {
            s5 += 's';
          }
    ...
    db.Ascents.read({key: key}, function (err, ascent) {
      ...
      Step(
        ...
        function (err, watchers, efforts, sends, imgURL) {
          ...
          s1 = _s.numberFormat(watchers, 0) + ' watcher';
          if (watchers !== 1) {
            s1 += 's';
          }

          s2 = _s.numberFormat(efforts, 0) + ' effort';
          if (efforts !== 1) {
            s2 += 's';
          }

          s3 = _s.numberFormat(sends, 0) + ' ascent';
          if (sends !== 1) {
            s3 += 's';
          }
      ...
      Step(
        ...
        function (err, imgURL, link) {
          ...
          _.each(session.ticks, function (tick, i) {
            ...
            if (tick.sent) {
              if (tick.grade) {
                var prefs = req.user && req.user.prefs ? req.user.prefs : defaultPrefs;
                var system = tick.type === 'r' ? prefs.grades.route: prefs.grades.boulder;
                var converter = gradeConverter[tick.type];
                desc += ' ' + converter.convert(tick.grade,
                    session.crag.country, system) + ' ✓';
              } else {
                desc += ' ✓';
              }
            } else {
        ...
        function (err, imgURL, link) {
          ...
          if (tick.sent) {
            if (tick.grade) {
              ...
              var system = tick.type === 'r' ? prefs.grades.route: prefs.grades.boulder;
              var converter = gradeConverter[tick.type];
              title += ' ' + converter.convert(tick.grade,
                  tick.crag.country, system) + ' ✓';
            } else {
              title += ' ✓';
            }
          } else {
