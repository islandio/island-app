...
exports.routes = function () {
  ...
  app.get('/service/media', function (req, res) {
    ...
    Step(
      function () {
        Events.feed({action: {type: 'post', query: {}}}, ['post'],
            {limit: limit, cursor: cursor}, this.parallel());
        Events.feed({action: {type: 'tick', query: {}}}, ['tick'],
            {limit: limit, cursor: cursor}, this.parallel());
      ...
      function (err, postFeed, tickFeed, sidebar, notes) {
        if (errorHandler(err, req, res)) return;

        var events = postFeed.events;
        events.items = [].concat(events.items, tickFeed.events.items);
        var items = [];
        Step(
          function () {
            ...
            var _this = _.after(events.items.length, this);
            _.each(events.items, function (e) {
              hasAccess(db, req.user, e.action, function (err, allow) {
                if (err) return _this(err);
                if (allow && e.action.medias.length !== 0) {
                  items.push(e);
                }
                _this();
              });
          ...
          function (err) {
            ...
            var profile = {
              member: req.user,
              transloadit: transloadit(req),
              content: _.extend(sidebar, {page: null, events: events})
            };
            if (notes) {
              profile.notes = {
                cursor: 1,
