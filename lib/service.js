...
exports.routes = function () {
  ...
  function getSidebarLists(member, requestor, cb) {
    ...
    Step(
      ...
      function (err, followers, followees, crags, ascents, recs) {
        ...
        recs = _.first(recs.sort(function (a, b) {
          return b.count - a.count;
        }), 16);
        var _this = _.after(recs.length, this);
        _.each(recs, function (r) {
          db.Members.read({_id: r._id}, function (err, m) {
            if (err) return _this(err);
            lists.recs.items.push({
              _id: m._id,
              username: m.username,
              role: m.role,
      ...
      function (err) {
        if (err) return cb(err);
        lists.recs.items.sort(function (a, b) {
    ...
    Step(
      function () {
        db.Ticks.list({type: 'b', sent: true, public: {$ne: false}},
            {sort: {created: -1}, inflate: {author: profiles.member,
            ascent: profiles.ascent, crag: profiles.crag}, limit: 5},
            this.parallel());
      ...
      function (err, boulders, routes, broadcasts) {
        if (err) return cb(err);
        var ticks = [].concat(boulders, routes);
        lists.broadcasts.items = broadcasts;

        // Check tick access.
        var _this = _.after(ticks.length, this);
        _.each(ticks, function (t) {
          hasAccess(db, undefined, t, function (err, allow) {
            if (err) return _this(err);
            if (allow) {
    ...
    Step(
      function () {
        ...
        if (req.params.slug === 'undefined') {
          ...
          if (!userId || _.isNaN(Number(userId))) return this('Invalid import slug');
