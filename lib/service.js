...
exports.routes = function (app) {
  ...
  app.get('/service/sessions.profile', function (req, res) {
    ...
    Step(
      function () {
        ...
        // Get sessions and notifications.
        db.Sessions.list({}, {sort: {date: -1, created: -1}, limit: 10,
      ...
      function (err, sessions, notes) {
        ...
        Step(
          function () {
            ...
            // Fill sub docs.
            db.fill(sessions, 'Comments', 'parent_id', {sort: {created: -1},
                limit: 5, reverse: true, inflate: {author: profiles.member}},
                this.parallel());
            db.fill(sessions, 'Actions', 'session_id', {sort: {index: 1}},
                this.parallel());
          },
          function (err) {
            if (com.error(err, req, res)) return;

            if (sessions.length === 0) return this();
            var _this = _.after(sessions.length, this);
            _.each(sessions, function (s) {
              db.fill(s.actions, 'Ticks', 'action_id', {sort: {index: 1},
          ...
          function (err) {
            ...
            var profile = {
              ...
              content: {
                sessions: {
                  cursor: 1,
                  more: sessions && sessions.length === 10,
                  items: sessions
                }
              }
            };
  ...
  // Home profile
  app.get('/service/posts.profile', function (req, res) {
  ...
  app.get('/service/profile.profile/:un', function (req, res) {
    ...
    db.Members.read({username: req.params.un.toLowerCase()},
      ...
      Step(
        function () {
          var par = (req.user && req.query.n !== '0') || mem.role === 2;
  ...
  // Posts
  app.get('/posts', _.bind(handler, undefined, function (req, res) {
