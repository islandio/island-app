...
exports.routes = function (app) {
  ...
  function getPublicSidebarLists(cb) {
    ...
    Step(
      ...
      function (err, island) {
        ...
        db.Posts.list({author_id: island._id}, {inflate: {author:
            profiles.member}, limit: 10, sort: {created: -1}}, this.parallel());
        db.Posts.count({author_id: island._id}, this.parallel());
      },
      function (err, crags, count) {
        if (err) return cb(err);
        if (crags.length === 0) {
          return this();
        }
        // lists.crags.items = datasets;
        // lists.crags.count = count;
  ...
  function getSplashLists(cb) {
    ...
    Step(
      function () {
        db.Ticks.list({sent: true}, {inflate: {author: profiles.user},
            limit: 10, sort: {date: -1}}, this);
      },
      function (err, ticks) {
        if (err) return cb(err);
        // TODO: check access
  ...
  app.get('/service/dashboard.profile', function (req, res) {
    if (!req.user) {
      return res.send(iutil.client({member: req.user, content: {}}));
    }
    var cursor = req.body.cursor || 0;
    var limit = req.body.limit || 5;
    var actions = parseEventActions(req, req.user ?
        ['session', 'post', 'crag', 'ascent']: ['post']);
    ...
    Step(
      function () {
        ...
        var query = req.user ?
            ...
            {public: true};
        Events.feed(query, actions, {limit: limit, cursor: cursor},
            this.parallel());
      ...
      function (err, feed, publicLists, lists, notes) {
        ...
        var profile = {
          member: req.user,
          transloadit: transloadit(req),
          content: _.extend(_.extend(lists, publicLists), {events: feed.events})
        };
        if (notes) {
          profile.notes = {
            cursor: 1,
  ...
  app.get('/service/trending', function (req, res) {
    ...
    Step(
      ...
      function (err, feed, publicLists, lists) {
        if (errorHandler(err, req, res)) return;
        lists = lists || {};
  ...
  app.get('/service/sessions.profile', function (req, res) {
    if (!req.user) {
      return res.send(iutil.client({member: null, content: {page: null}}));
    }
    var cursor = req.body.cursor || 0;
    var limit = req.body.limit || 5;
        ...
        Step(
          ...
          function (err) {
            ...
            var profile = {
              member: req.user,
              transloadit: transloadit(req),
              content: _.extend(lists, {page: {author: req.user, ticks: ticks}})
            };
            if (notes) {
              profile.notes = {
                cursor: 1,
  ...
  // Tick profile
  app.get('/service/tick.profile/:key', function (req, res) {

    Step(
      function () {

        // Get tick and notifications.
        db.Ticks.read({key: req.params.key},
            {inflate: {author: profiles.member, crag: profiles.crag,
            session: profiles.session, ascent: profiles.ascent}},
            req.user ? this.parallel(): this);
  ...
  app.get('/service/crag.profile/:y/:g', function (req, res) {
    ...
    db.Crags.read({key: key, forbidden: {$ne: true}}, function (err, crag) {
      ...
      Step(
        ...
        function (err, weather, events, ascents, sub, notes) {
          if (errorHandler(err, req, res)) return;
  ...
  app.get('/service/films.profile', function (req, res) {
    ...
    Step(
      ...
      function (err, events, lists, notes) {
        ...
        // Write profile.
        var content = {page: null, events: events};
        var profile = {
          member: req.user,
          transloadit: transloadit(req),
          content: _.extend(lists, content)
        };
        if (notes) {
          profile.notes = {
            cursor: 1,
