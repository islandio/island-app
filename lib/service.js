...
var films_static = fs.readFileSync('public/templates/films.html', 'utf8');
// var blog_static = fs.readFileSync('public/templates/blog.html', 'utf8');
// var profile_static = fs.readFileSync('public/templates/profile.html', 'utf8');
var ticks_static = fs.readFileSync('public/templates/ticks.html', 'utf8');
var crags_static = fs.readFileSync('public/templates/crags.html', 'utf8');
var crag_static = fs.readFileSync('public/templates/crag.html', 'utf8');
var ascent_static = fs.readFileSync('public/templates/ascent.html', 'utf8');
var session_static = fs.readFileSync('public/templates/rows/session.html', 'utf8');
var tick_static = fs.readFileSync('public/templates/rows/tick.html', 'utf8');
var post_static = fs.readFileSync('public/templates/rows/post.html', 'utf8');
...
exports.routes = function () {
    ...
    var get = {
      _followers: function (cb) {
        ...
        db.Subscriptions.list({subscribee_id: member._id, 'meta.style': 'follow',
            ...
            inflate: {subscriber: profiles.member}}, function (err, docs) {
      ...
      _followees: function (cb) {
        ...
        db.Subscriptions.list({subscriber_id: member._id, 'meta.style': 'follow',
            'meta.type': 'member'}, {sort: {created: -1},
            inflate: {subscribee: profiles.member}}, function (err, docs) {
    ...
    Step(
      ...
      function (err, r, body) {
        ...
        return cb(null, ['https://' + app.get('CARTODB_USER') +
            ...
            location.latitude, location.longitude, 200, 200].join('/') +
            '.png');
      }
    );
  }
    ...
    Step(
      function () {
        ...
        if (req.params.slug === 'undefined') {
          ...
          var userId = req.params.slug.split('-').slice(-1)[0];
          if (!userId || _.isNaN(Number(userId))) {
            return this({code: 403, message: 'Invalid import slug'});
          }
          lib8a.getTicks(userId, _.bind(function (err, ticks) {
        ...
        function (err, allow) {
          ...
          Step(
            function () {
              ...
              // Get sidebar.
              getSidebar(mem, req.user, ['watchees'], this.parallel());
            ...
            function (err, ticks, sidebar, notes) {
              ...
              Step(
                function () {
                  ...
                  function _count(d, type, cb) {
                    db[type].count({parent_id: d._id}, function (err, cnt) {
                      if (err) cb(err);
                      d[type.toLowerCase() + '_cnt'] = cnt;
                      cb();
                    });
                  }

                  // Fill ticks.
                  _.each(ticks, _.bind(function (t) {
                    _count(t, 'Medias', this.parallel());
                    _count(t, 'Hangtens', this.parallel());
                    _count(t, 'Comments', this.parallel());
                    t.ascent = {name: t.ascent.name, key: t.ascent.key,
                        grades: t.ascent.grades};
                    t.crag = {name: t.crag.name, key: t.crag.key};
                  }, this));
                },
                function (err) {
                  if (errorHandler(err, req, res)) return;
  ...
  app.get('/service/crags', function (req, res) {
    ...
    Step(
      function () {
        ...
        // Get sidebar.
        getSidebar(req.user, req.user, ['ticks', 'watchees'], this.parallel());
  ...
  app.get('/', _.bind(handler, undefined, function (req, res) {
    Step(
      ...
      function (err, sidebar) {
        ...
        var ctx = {
          app: {
            profile: {content: _.extend(sidebar, {page: null})}
          }
        };
        res.render('static', {
          url: process.env.tunnelURL || app.get('HOME_URI'),
  ...
  app.get('/signin', function (req, res) {
    ...
    handler(function (req, res) {
      res.render('static', {
        title: 'Signin',
        url: (process.env.tunnelURL || app.get('HOME_URI')) + '/signin',
  ...
  app.get('/signup', function (req, res) {
    ...
    handler(function (req, res) {
      res.render('static', {
        title: 'Signup',
        url: (process.env.tunnelURL || app.get('HOME_URI')) + '/signup',
  ...
  app.get('/crags', _.bind(handler, undefined, function (req, res) {
    res.render('static', {
      title: 'Crags',
      url: (process.env.tunnelURL || app.get('HOME_URI')) + '/crags',
      body: _.template(crags_static)(),
      root: app.get('ROOT_URI')
    });
  }, 'folder', true));

  // Import
  app.get('/import', _.bind(handler, undefined, function (req, res) {
    ...
    Step(
      ...
      function (err, sidebar, posts) {
        ...
        db.fill(posts, 'Medias', 'parent_id', {sort: {created: -1}},
          ...
          var ctx = {
            app: {profile: {content: _.extend(sidebar, {page: null,
                static_posts: posts})}}
          };
          ...
          res.render('static', {
            url: (process.env.tunnelURL || app.get('HOME_URI')) + '/films',
            title: 'Films',
            ...
            body: _.template(films_static).call(ctx)
          });
        });
      }
    );
  ...
  app.get('/privacy', _.bind(handler, undefined, function (req, res) {
    Step(
      ...
      function (err, sidebar) {
        ...
        res.render('static', {
          url: (process.env.tunnelURL || app.get('HOME_URI')) + '/privacy',
          ...
          root: app.get('ROOT_URI'),
          assets: process.env.tunnelURL || app.get('HOME_URI'),
          body: _.template(privacy_static).call(ctx)
        });
      }
    );
  }, 'folder'));
  ...
  app.get('/crags/:y/:g', _.bind(handler, undefined, function (req, res) {
    ...
    db.Crags.read({key: key}, function (err, crag) {
      ...
      var desc = crag.description && crag.description !== '' ?
          crag.description + '\n\n': '';
      if (crag.bcnt > 0)
        desc += _.numberFormat(crag.bcnt, 0) + ' boulder problems';
      if (crag.rcnt > 0) {
        if (desc !== '') desc += ', ';
        desc += _.numberFormat(crag.rcnt, 0) + ' routes';
      }

      getStaticMap(crag.location, function (err, imgURL) {
        if (errorHandler(err, req, res)) return;
        res.render('static', {
          url: (process.env.tunnelURL || app.get('HOME_URI')) + '/crags/' + key,
          title: title,
          description: desc,
          medias: [{image: {type: 'image', url: imgURL, ssl_url: imgURL, meta:
