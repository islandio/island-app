...
exports.routes = function (app) {
  ...
  function getSplashLists(cb) {
    ...
    Step(
      ...
      function (err, mem) {
        ...
        db.Posts.list({author_id: mem._id}, {inflate: {author:
            profiles.member}, limit: 3, sort: {created: -1}}, this);
      },
      function (err, posts) {
        if (err) return cb(err);
        lists.posts.items = posts;
        cb(null, lists);
      }
    );
  }
  ...
  app.get('/service/dashboard', function (req, res) {
    ...
    Step(
      function () {
        ...
        // Get events.
        var query = req.user ?
            {member_id: req.user._id, subscriber_id: req.user._id}:
            {public: true};
  ...
  app.get('/service/sessions', function (req, res) {
    if (!req.user) {
  ...
  // User ticks profile
  app.get('/service/ticks', function (req, res) {
    if (!req.user) {
      return res.send(iutil.client({member: req.user, content: {}}));
    }
    var cursor = req.body.cursor || 0;
        ...
        Step(
          ...
          function (err) {
            ...
            var profile = {
              ...
              transloadit: transloadit(req),
              content: _.extend(_.extend(lists, publicLists),
                  {page: {author: req.user, ticks: ticks}})
            };
            if (notes) {
              profile.notes = {
                cursor: 1,
  ...
  app.get('/service/crag/:y/:g', function (req, res) {
    ...
    db.Crags.read({key: key, forbidden: {$ne: true}}, function (err, crag) {
      ...
      Step(
        ...
        function (err, weather, feed, ascents, sub, notes) {
          if (errorHandler(err, req, res)) return;
  ...
  app.get('/trending', _.bind(handler, undefined, function (req, res) {
    ...
    Step(
      ...
      function (err, feed, publicLists) {
        ...
        var html = '<h2>Public content now trending on Island:</h2>';
  ...
  // Session permalink
  app.get('/:un/sessions/:k', function (req, res) {
    if (!req.user) {
      return res.redirect('/');
    }
    handler(function (req, res) {
  ...
  // Tick permalink
  // TODO: check req.user against req.param.un
  app.get('/:un/ticks/:k', function (req, res) {
    if (!req.user) {
      return res.redirect('/');
    }
    handler(function (req, res) {
