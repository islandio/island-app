...
exports.routes = function (app) {
  ...
  app.get('/service/dashboard.profile', function (req, res) {
    ...
    var actions = req.query.actions || ['session', 'post'];
    ...
    Step(
      function () {
        ...
        Events.feed(req.user._id, actions, {sort: {date: -1, created: -1},
    ...
    db.Members.read({username: req.params.un.toLowerCase()},
      ...
      Step(
        function () {
          ...
          query.subscribee_id = mem._id;
          Events.feed(mem._id, actions, {sort: {date: -1, created: -1},
            limit: limit, cursor: cursor, query: query}, 
            par ? this.parallel(): this);
          if (mem.role === 2)
